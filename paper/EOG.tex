...
%AANPASSINGEN: 
% - Alle probleem-oplossingen structuren zijn gegroepeerd (probleem - oplossing structuur met subsubsections, misschien nog zien of dit wel aangewezen is.).
% - Secties geschreven door 'Pieter Verlinden' zijn bekeken en aangepast waar nodig. (zoals afgesproken op vergadering met begeleiders).

% Opmerkingen door Pieter:
% - Alles is herlezen en aangepast waar nodig (bv. te lange zinnen, dt fouten (misschien nog niet allemaal?)) Figuren zijn ook wat kleiner gemaakt zodat er maar 8 paginas zijn. Ik denk persoonlijk ook dat de manier om Probleem: en Oplossing: niet echt gangbaar is voor papers en dat de tekst zelf dit impliceert. (In de tekst staat altijd: een probleem is bla bla bla en daarna: dit kan opgelost worden door bla bla bla).

%TODO: Alles nog eens grondig nalezen op spel/grammaticale fouten en vergelijken met de eerst ingediende versie (in mapje backup).
...
%bijkomstig --> bijkomend
%dit potentiaal --> deze potentiaal
%weaselwords
%2de soort ruis --> interferentie
